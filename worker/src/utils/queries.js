export const kda = async (DB, control, order) => {
  const games = 10;
...
export const playerChampionWR = async (DB, control, order) => {
  const games = 10;
  const o = order.toUpperCase() === "ASC" ? "ASC" : "DESC";
  return await DB.prepare(`
    SELECT
...
export const matchDuration = async (DB, control, order) => {
  const o = order.toUpperCase() === "ASC" ? "ASC" : "DESC";
  return await DB.prepare(`
    SELECT
      p.riot_name, p.riot_tag, p.lol_picture, p.lol_region,
...
export const playerWR = async (DB, control, order) => {
  const o = order.toUpperCase() === "ASC" ? "ASC" : "DESC";
  const games = 10;
  return await DB.prepare(`
    SELECT
